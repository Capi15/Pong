class MenuSceneDesktop extends Phaser.Scene {
  ...
  create() {
    ...
    this.timmerText = this.add.text(
      500,
      100,
      "O jogo iniciará em " + this.formatTime(this.initialTime) + " segundos"
    );
    ...
    socket.on("mostraJogadores", (listaJogadores) => {
      ...
      var y = 250;
      let id = 0;
      this.stringListaNomes = this.addText(" A atualizar...");
    ...
    socket.on("playerCount", (playerCount) => {
      this.totalPlayers = this.setText("Na Fila -> " + playerCount + "/6");
